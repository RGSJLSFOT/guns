var DateUtils = {
    ...
    TransformationWeek :function (dayCycle){
...
        }
    ...
    getHour :function (startTime, endTime) {
        ...
        startTime = new Date((reDate.test(startTime) ? startTime : new Date().toLocaleDateString().split('/').join('-') + " 09:00:00").replace(/-/g, '/'));
        endTime = new Date((reDate.test(endTime) ? endTime : new Date().toLocaleDateString().split('/').join('-') + " 18:00:00").replace(/-/g, '/'));
        var ms = endTime.getTime() - startTime.getTime();
        if (ms < 0) return 0;
        var deltmins = (ms/1000/60)%60>=30?0.5:0;
        if(startTime.getHours()< 12 && endTime.getHours()> 13)
        	return Math.floor(ms / 1000 / 60 / 60) - 1+deltmins;
        else
        	return Math.floor(ms / 1000 / 60 / 60)+deltmins;
    }
